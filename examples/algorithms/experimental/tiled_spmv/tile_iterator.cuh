...
class TileIterator {
 public:
  ...
  __device__ TileIterator(const graph_t _graph,
    ...
    TileIdx _tile_idx(0, 0, hierarchy);
    tile_idx = _tile_idx;
  }
...
class MatrixTileIterator
    : public TileIterator<graph_t, vector_t, shmem_t, indexer_t, hierarchy> {
 public:
  __device__ __forceinline__ MatrixTileIterator(const graph_t _graph,
      ...
      : TileIterator<graph_t, vector_t, shmem_t, indexer_t, hierarchy>(
            _graph,
            _input,
            _output,
            _tile_row_size,
  ...
  __device__ __forceinline__ void process_all_tiles() {
    for (this->tile_idx.row == 0;
         this->tile_idx.row < this->tile_indexer.num_row_tiles(hierarchy);
         this->tile_idx.row++) {
      for (this->tile_idx.col = 0;
           this->tile_idx.col < this->tile_indexer.num_col_tiles(hierarchy);
           this->tile_idx.col++) {
        print_device("Processing tile (%d, %d)\n", (int)this->tile_idx.row,
                      (int)this->tile_idx.col);
      }
    }
  }
};