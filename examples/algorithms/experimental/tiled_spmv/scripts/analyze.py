...
for filename_short in os.listdir(profiledir):
    ...
    filedata = pd.read_csv(filename_long, skiprows=2)

    # Building the column headers for the first time
    if suitesparse_data.empty:
        column_names = filedata.columns.values.tolist()
...
# suitesparse_data[suitesparse_data.columns[1:]
#                  ] = suitesparse_data[suitesparse_data.columns[1:]].applymap(make_numeric)
