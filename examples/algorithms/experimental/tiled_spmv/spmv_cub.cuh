...
double spmv_cub(csr_t& A, vector_t& input, vector_t& output)
{
  ...
  CHECK_CUDA(cub::DeviceSpmv::CsrMV(d_temp_storage, temp_storage_bytes, A.nonzero_values.data().get(),
                         A.row_offsets.data().get(), A.column_indices.data().get(), input.data().get(), output.data().get(),
                         A.number_of_rows, A.number_of_columns, A.number_of_nonzeros));
  ...
  CHECK_CUDA(cub::DeviceSpmv::CsrMV(d_temp_storage, temp_storage_bytes, d_values, d_row_offsets, d_col_idx, input.data().get(), output.data().get(),
                         A.number_of_rows, A.number_of_columns, A.number_of_nonzeros, 0, true));
  CHECK_CUDA(cudaDeviceSynchronize());
  timer.end();
